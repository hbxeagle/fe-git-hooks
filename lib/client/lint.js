(function() {
    ...
    var jshint = require('jshint').linter;
    var fs = require('fs');
    ...
    exports.lintFiles = function(files, options) {
        var gspConfig = path.join(options.base, '.fehooksconfig');
        if (fs.existsSync(gspConfig)) {
            options.lintConfigs = JSON.parse(fs.readFileSync(gspConfig)).lint;
